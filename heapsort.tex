...
Dies kann man erreichen, indem man die Schlüssel $k_{\lfloor N/2\rfloor}$ bis $k_1$ versickern lässt. 
...
\begin{algorithmic}[1]
  ...
  \State \texttt{child.right.parent} $=$ \texttt{parent}
  \State \texttt{parent.right.parent} $=$ \texttt{child}
  ...
  \State \texttt{child.left.parent} $=$ \texttt{parent}
  \State \texttt{parent.left.parent} $=$ \texttt{child}
