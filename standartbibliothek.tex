...
\begin{equation}
\mathrm{cos}\left(x\right)=\sum_{l=0}^{\infty} \left(-1\right)^{l} \dfrac{x^{2l}}{\left(2l\right)!}
\end{equation}
...
die ernäherung des \texttt{cos} Funktions selbst mit summieren ihren Taylor Reihenfolge
bis \texttt{n}
\begin{equation}
\mathrm{cos}_{approx}\left(x,n\right)=\sum_{l=0}^{n} \left(-1\right)^{l} \dfrac{x^{2l}}{\left(2l\right)!}
\end{equation}
...
Die weiteren erreichbaren  Funktionen zählen wir auf im Tabelle \ref{math}. 
\begin{table}[t]
