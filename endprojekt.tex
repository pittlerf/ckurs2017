...
deswegen die Ganze Zeit skaliert mit \texttt{n*log(n)}. Das ist für groß genug \text{n} ist
...
Aber es gibt nur eine Karte für Sie, und die Eltern dürfen nicht gehen, sie müssen sich 
...
geb die Möchkligkeit für ihre Kinder zum Konzert zu gehen. In anderen Wörten, er
...
wie unseres Problem: den ersten freien Platz zu finden. Nachdem Beispiel ist es 
...
void queue_put( heap *data, queue ** first){
   ...
   queue *temp=newqueue(data);//Das wird das erste Element sein
   temp->next=(*first);//Das vorherige Erste wird jetzt das zweite
   (*first)=temp;
}
//Loescht ein Element aus dem queue
heap *queue_rem( queue **first){
   queue *temp;
   heap *ret;
   if ((*first) == NULL){
...
Zusamennfassen wir haben drei Möglichkeiten um das $heap$ Eigenschaft wiederherstellen
